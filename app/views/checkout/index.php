...
<style>
    ...
    .stepper-line.inactive {
        background-color: #0A3167 !important;
    }
    .stepper-text.active {
        color: #C5A572;
    }
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center max-w-3xl mx-auto py-8">
        <div class="flex flex-col items-center">
            <div class="w-10 h-10 rounded-full stepper-circle active flex items-center justify-center">
                <i class="fas fa-shopping-cart stepper-icon"></i>
            </div>
            <span class="text-sm mt-2 stepper-text active font-medium">Cart</span>
        </div>
        <div class="flex-1 h-1 stepper-line active mx-2"></div>
        <div class="flex flex-col items-center">
            <div class="w-10 h-10 rounded-full stepper-circle active flex items-center justify-center">
                <i class="fas fa-address-card stepper-icon"></i>
            </div>
            <span class="text-sm mt-2 stepper-text active font-medium">Details</span>
        </div>
        <div class="flex-1 h-1 stepper-line inactive mx-2"></div>
        <div class="flex flex-col items-center">
            <div class="w-10 h-10 rounded-full stepper-circle inactive flex items-center justify-center">
                <i class="fas fa-credit-card stepper-icon"></i>
            </div>
            <span class="text-sm mt-2 stepper-text inactive">Payment</span>
        </div>
        <div class="flex-1 h-1 stepper-line inactive mx-2"></div>
        <div class="flex flex-col items-center">
            <div class="w-10 h-10 rounded-full stepper-circle inactive flex items-center justify-center">
            ...
            <span class="text-sm mt-2 stepper-text inactive">Complete</span>
        </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="lg:w-2/3">
            ...
            <?php if (isset($_SESSION['flash_message'])): ?>
                <div class="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded-lg mb-8 shadow-sm" role="alert">
                ...
                <form id="checkout-form" action="<?= \App\Core\View::url('checkout/process') ?>" method="post" enctype="multipart/form-data">
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <select name="address_id" id="address_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-gray-500" required>
                        ...
                        <div>
                            ...
                            <a href="<?= \App\Core\View::url('user/address') ?>" class="text-blue-600 hover:text-blue-800 font-medium">Add New Address</a>
                        </div>
                    </div>

                    <!-- Payment Information -->
                    <h2 class="text-xl font-semibold text-gray-900 mt-8 mb-6">Payment Information</h2>
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label for="payment_method_id" class="block text-gray-700 mb-2">Payment Method</label>
                            <select name="payment_method_id" id="payment_method_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-gray-500" required>
                    ...
                    <div id="bank-transfer-fields" class="payment-method-fields hidden mt-6">
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" name="transaction_id" id="transaction_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-gray-500">
                            </div>
                            <div>
                                <label for="payment_screenshot" class="block text-gray-700 mb-2">Payment Screenshot</label>
                                <input type="file" name="payment_screenshot" id="payment_screenshot" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-gray-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between mt-8">
                        <a href="<?= \App\Core\View::url('cart') ?>" class="px-6 py-3 border border-gray-500 text-gray-500 rounded-lg hover:bg-gray-100">
                            Back to Cart
                        </a>
                        <button type="submit" class="px-8 py-3 bg-primary text-white rounded-lg hover:bg-accent">
                            Continue to Payment
                        </button>
                    </div>
                </form>
                    ...
                    <?php foreach ($cartItems as $item): ?>
                        <div class="flex">
                            ...
                            <div class="ml-4 flex-1">
                                <h4 class="font-medium text-gray-900"><?= htmlspecialchars($item['product']['product_name']) ?></h4>
                ...
                <div class="border-t border-gray-200 pt-4 space-y-3">
                    ...
                    <div class="border-t border-gray-200 pt-3 mt-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-xl text-gray-900">₹<?= number_format($finalTotal, 2) ?></span>
                        </div>
                    </div>
                </div>
            </div>
